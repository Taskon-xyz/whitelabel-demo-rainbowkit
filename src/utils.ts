...
export const signMessage = async (clientId: string, loginType: LoginType, value: string): Promise<{
  signature: string;
  timestamp: number;
}> => {
 ...
 */
export const signMessageNode = (clientId: string, loginType: LoginType, value: string): {
  signature: string;
  timestamp: number;
} => {
  const timestamp = Date.now();
